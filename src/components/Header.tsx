...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (!isManualScrolling.current && window.scrollY < 10) {
  ...
  return (
    <header 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        isScrolled ? 'bg-zinc-950/80 backdrop-blur-md border-b border-white/5 py-4' : 'bg-transparent py-8'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between">
        ...
        >
          <div className="w-9 h-9 rounded-lg flex items-center justify-center border transition-all shadow-[0_0_15px_rgba(14,165,233,0.1)] group-hover:shadow-[0_0_20px_rgba(14,165,233,0.2)] bg-sky-500/10 border-sky-500/20 group-hover:border-sky-500/50">
            <Terminal size={18} className="text-sky-500 transition-transform group-hover:scale-110" />
          </div>
          <span className="text-xl font-black tracking-tight transition-colors text-white group-hover:text-sky-400">linus.dev</span>
        </div>
        ...
        <nav className="hidden lg:flex items-center gap-10">
          {navLinks.map((s) => {
            ...
            return (
              ...
              >
                {s}
                <span className={`absolute -bottom-1 left-0 h-[1px] bg-sky-500 transition-all duration-500 ${
                  isActive ? 'w-full shadow-[0_0_8px_rgba(14,165,233,0.8)]' : 'w-0 group-hover:w-full'
                }`}></span>
        ...
        <div className="hidden lg:block">
          ...
          >
            ...
            <div className="flex items-center gap-3 transition-all duration-300 relative z-10">
              <Mail size={14} className="text-sky-500 group-hover:text-white transition-colors" />
              <span>Mail Me</span>
            </div>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 top-[70px] bg-zinc-950 z-40 lg:hidden flex flex-col p-8 gap-8 animate-in fade-in slide-in-from-top-4 duration-300">
          {[Section.Hero, ...navLinks].map((s) => (
            <button
              key={s}
              onClick={() => scrollTo(s)}
              className={`text-4xl font-black transition-all capitalize text-left active:translate-x-2 ${
                activeSection === s ? 'text-sky-500' : 'text-zinc-700 hover:text-white'
              }`}
            >
              {s}
            </button>
          ...
          <a 
            href="mailto:bhattaraisunil76@gmail.com" 
            className="mt-4 p-6 bg-zinc-900 border border-sky-500/20 rounded-2xl flex items-center justify-between group active:bg-zinc-800"
          >
            <span className="text-lg font-bold text-white uppercase tracking-widest">Connect</span>
