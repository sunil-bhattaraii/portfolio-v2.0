...
const FloatingNav: React.FC = () => {
  ...
  return (
    <>
      {isVisible && (
        <div className="fixed right-6 top-1/2 -translate-y-1/2 z-[90] hidden lg:flex flex-col items-center gap-4">
            ...
            {SECTION_ORDER.map((s, i) => (
              <div
                ...
                className={`w-1 h-1 rounded-full transition-all duration-500 ${
                  i === currentIndex
                    ? 'bg-sky-500 scale-150 shadow-[0_0_8px_#0ea5e9]'
                    : 'bg-zinc-800'
                }`}
              />
            ))}
          ...
          <button
            ...
            className={`w-12 h-12 rounded-xl border border-white/10 glass flex items-center justify-center transition-all group ${
              !hasPrev
                ...
                : 'opacity-100 hover:border-sky-500/50 text-zinc-500 hover:text-sky-400'
            }`}
          >
            <ChevronUp
              size={20}
          ...
          <div className="mt-4 overflow-hidden h-4 w-20 flex justify-center">
            <span className="text-[8px] font-black uppercase tracking-[0.3em] text-zinc-600 whitespace-nowrap">
              {activeSection}
            </span>
          </div>
        </div>
