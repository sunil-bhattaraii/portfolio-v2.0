...
const ExperienceCard: React.FC<ExperienceCardProps> = ({ exp, index }) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-8 p-8 md:p-12 bg-zinc-900/30 border border-white/5 hover:bg-zinc-900/50 hover:border-sky-500/30 transition-all rounded-2xl overflow-hidden"
    >
      ...
      <div className="space-y-4">
        ...
        <h3 className="text-xl font-bold text-white group-hover:text-sky-400 transition-colors">{exp.company}</h3>
