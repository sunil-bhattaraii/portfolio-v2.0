...
interface AIAssistantClientProps {
  onSendMessage: (messages: ChatMessage[], userMessage: string) => Promise<string>;
}
...
const AIAssistantClient: React.FC<AIAssistantClientProps> = ({ onSendMessage }) => {
  ...
  const handleSendMessage = async (text: string = inputValue) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", text: "Connection error. Please check your network protocol." },
      ]);
    } finally {
      setIsLoading(false);
    }
                        ...
                        <ReactMarkdown
                          components={{
                            ...
                            strong: ({ node: _node, ...props }) => (
                              <strong className="font-bold text-sky-400" {...props} />
                            ),
